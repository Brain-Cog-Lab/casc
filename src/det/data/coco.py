...
class COCODataset(Dataset):
    ...
    def __init__(self, 
        ...
        # self.ids = list() # 新加的
        # for line in open('/home/hexiang/VOC0712/VOCdevkit/VOC2007/ImageSets/Main/test.txt', 'r'):
    ...
    def __getitem__(self, index):
        ...
        if self.image_set == 'val2017' or self.image_set == 'test2017':
            return im, gt, h, w, scale, offset, self.ids[index]
        else:
            return im, gt
    ...
    def pull_image(self, index):
        ...
        # img_file = os.path.join(self.data_dir, self.image_set,
        #                        '{:06}'.format(id_) + '.jpg')
        #此处是第一个   现在是想让voc用coco的评价指标 # 注意上面的指标（label顺序也要换）
        # img_file = '/home/hexiang/VOC2COCO/COCO/val2017/' + id_ + ".jpg"
        img = cv2.imread(img_file)
        if self.json_file == 'instances_val5k.json' and img is None:
            img_file = os.path.join(self.data_dir, 'train2017',
                                    '{:012}'.format(id_) + '.jpg')
...
if __name__ == "__main__":
    ...
    img_size = 640
    dataset = COCODataset(
                data_dir='/home/hexiang/VOC2COCO/COCO/',
                img_size=img_size,
                image_set='val2017',
    ...
    for i in range(5000):
        ...
        # cv2.imshow('gt', image)
        cv2.imwrite('/home/hexiang/VOC2COCO/'+ str(i)+'.jpg', image)
        # cv2.waitKey(0)
