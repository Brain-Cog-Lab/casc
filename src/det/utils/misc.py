...
class CollateIndFunc(object):
    def __call__(self, batch):
        ...
        for sample in batch:
            ...
            h.append(sample[2])
            w.append(sample[3])
            scale.append(sample[4])
            offset.append(sample[5])
            indx.append(sample[6])
        images = torch.stack(images, 0)     # [B, C, H, W]
        # h = torch.stack(h, 0)
        # w = torch.stack(w, 0)
        # scale = torch.stack(scale, 0)
        # offset = torch.stack(offset, 0)
        # indx = torch.stack(indx, 0)
        return images, targets, h, w, scale, offset, indx
